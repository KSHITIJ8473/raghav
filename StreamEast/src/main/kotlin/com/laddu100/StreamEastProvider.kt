...
class StreamEastProvider : MainAPI() {
    ...
    private var lastFirebaseLoadTime: Long = 0L
    private val FIREBASE_CACHE_TTL_MS = 5 * 60 * 1000L  // 5 minutes
    ...
    private suspend fun loadFirebaseUrl() {
        ...
        if (isUrlLoaded && now - lastFirebaseLoadTime < FIREBASE_CACHE_TTL_MS) return
        try {
            ...
            url?.let {
                if (it.isNotEmpty()) {
                    ...
                    isUrlLoaded = true
                    lastFirebaseLoadTime = now
                }
            }
        } catch (e: Exception) {
            // Keep stale URL if we have one; retry after TTL
            lastFirebaseLoadTime = now
        }
    }
